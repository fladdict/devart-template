...
#define SETTINGS_FILE_NAME "appSettings.xml"
...
void testApp::setup(){
    ...
    XML.getValue("SENSOR0:POSITION:X", -0.1);
    XML.getValue("SENSOR0:POSITION:Y", -0.1);
    XML.getValue("SENSOR0:ROTATION", 45);
    XML.getValue("SENSOR0:SCAN_ANGLE", 270);
    XML.getValue("SENSOR0:ADDRESS", "192.168.0.10");
    XML.getValue("SENSOR0:PORT", 10940);*/
...
void testApp::setupGui(){
    ...
    gui.add(paramIsExplosionEnabled.set("Explosion", true));
    gui.add(paramExplosionSparkParticleNum.set("Spark Num", 50, 0, 200));
    gui.add(paramExplosionSmokeParticleNum.set("Smoke Num", 10, 0, 100));
    ...
    gui.add(paramAutoTransitionProbability.set("Auto Location Move",0.1,0.00,1.00));
    
    gui.add(paramLabelSplitter.set(""));
    gui.add(paramLabel4.set("Browser"));
    gui.add(paramIsBrowserMode.set("Browser Control Mode", false));
    gui.add(paramIsWireframeMode.set("Show Wireframe", false));
    gui.add(paramIsShowSensorTile.set("Show Sensors", false));
}


void testApp::exit(){
    floorSystem->tearDown();
    browser.tearDown();
}
...
{
    GiantStepLog newLog;
    newLog.position = ofVec2f(mouseX, mouseY);
    newLog.timeStamp = ofGetSystemTimeMicros();
    
    //Prevent rensha
    vector<GiantStepLog>::iterator it=giantStepLogs.begin();
    while(it!=giantStepLogs.end()){
        int diff = newLog.timeStamp - it->timeStamp;
        int posDiff = newLog.position.distance(it->position);
        if(diff<200000 && posDiff<150){
            return;
        }
        it++;
    }
    
    giantStepLogs.push_back(newLog);
    if(giantStepLogs.size()>20){
        giantStepLogs.erase(giantStepLogs.begin());
    }
    ...
    if(paramIsExplosionEnabled && ofRandom(0, 1)<0.5){
        sparkEmitter.addParticles(mouseX, mouseY, paramExplosionSparkParticleNum);
        crackEmitter.addParticle(mouseX, mouseY);
        smokeEmitter.addParticles(mouseX, mouseY, paramExplosionSmokeParticleNum);
...
void testApp::removeParticles(){
    crackEmitter.removeAllParticles();
    smokeEmitter.removeAllParticles();
    sparkEmitter.removeAllParticles();
    airPlane.resetPosition();
    cloud.resetPosition();
}
...
void testApp::onFloorTouched(ofVec2f &e){
    float xx = e.x * ofGetHeight();
    float yy = e.y * ofGetHeight();
    // cout << " " << xx << " " << yy << endl;
