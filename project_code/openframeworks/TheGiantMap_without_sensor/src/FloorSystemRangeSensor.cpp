...
string FloorSystemRangeSensor::ofxTrimStringRight(string str) {
    size_t endpos = str.find_last_not_of("\t\r\n");
    return (string::npos != endpos) ? str.substr( 0, endpos+1) : str;
}


string FloorSystemRangeSensor::ofxTrimStringLeft(string str) {
    size_t startpos = str.find_first_not_of("\t\r\n");
    return (string::npos != startpos) ? str.substr(startpos) : str;
}


string FloorSystemRangeSensor::ofxTrimString(string str) {
    return ofxTrimStringLeft(ofxTrimStringRight(str));;
}


string FloorSystemRangeSensor::ofxGetSerialString(ofSerial &serial, char until) {
    static string str;
    stringstream ss;
    char ch;
    int ttl=1000;
